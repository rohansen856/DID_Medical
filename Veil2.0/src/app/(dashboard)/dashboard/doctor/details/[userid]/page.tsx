          ...
          {!unlockedSections.has(index) && (
            <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-30 rounded-lg">
              <Button
                ...
                aria-label={clickedSections.has(index) ? `Unlock ${section.title}` : `Request access to ${section.title}`}
              >
                {clickedSections.has(index) ? `Unlock ${section.title}` : `Request Access`}
              </Button>
            </div>
          )}
        </div>
